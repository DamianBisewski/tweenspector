...
class TweetsData:
    ...
    def generate_user_stats(self, option):
        ...
        def generate_statistics_chart():
            ...
            values_name = {'avglikes': "Średnia ilość like'ów",
                           'maxlikes': "Największa liczba like'ów",
                           # 'minlikes': "Najmniejsza liczba like'ów",  # generally equals 0
                           # 'medianlikes': "Mediana liczby like'ów",  # similar to average likes
                           'avgretweets': "Średnia liczba retweet'ów",
                           'maxretweets': "Największa liczba retweet'ów",
                           # 'minretweets': "Najmniejsza liczba retweet'ów",  # generally equals 0
                           # 'medianretweets': "Mediana liczby retweetów",  # similar to average retweets
                           }
            values_count = []
            for name in list(values_name.keys()):
                values_count.append(account_stats[name])
            plt.barh(list(values_name.values()), values_count, color="blue")
        ...
        def generate_hashtag_chart():
            ...
            hashtag_count = list(hashtag_dict.values())
            hashtag_name = list(hashtag_dict.keys())

            plt.figure(figsize=(12, 5))

            plt.barh(hashtag_name, hashtag_count)
            plt.title("Wykres ilości wystąpień hashtagów w zależności od jego nazwy")
